...
export async function generateMetadata({params}: { params: Promise<{ gameId: string }> }) {
    ...
    return {
        title: game ? `${game.title.en} | mustakuusi` : "Game not found",
    };
}

export default async function GamePageEN({params}: { params: Promise<{ gameId: string }> }) {
    const {gameId} = await params;
    const gamesList = localGames as GameData[];
    ...
    const gameStats = [
        {
            ...
            label: {id: "Karakter", en: "Character(s)"},
        },
    ];
    ...
    return (
        <div>
            ...
            />
            <Characters characters={filteredCharacters} characterSearch={`CHARACTERS IN GAME`}/>
            <Screenshots screenshots={game.screenshots} title="GAME SCREENSHOTS"/>
